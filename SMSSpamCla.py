...
class SMSSpamCla:
	...
	def train_svm(self):
		self.cla = SVC(kernel = self.kernel, class_weight = 'balanced').fit(self.X, self.Y)
	...
	def train(self, model_id, kernel):
		self.kernel = kernel
...
if __name__ == "__main__":
	...
	for kernel in ['linear','rbf']:
		...
		model_name = cla.train(3, kernel)
		res = cla.get_result()
		label = model_name + "+" + "tok1"+" kernel:"+kernel
